...
class TestTopKTripleExtractor:
    def test_returns_k_triples(self, injector):
        ...
        triples = np.random.rand(10, 16)
        fake_sim = np.random.rand(1, 10)
        with patch("kaping.entity_injection.cosine_similarity", return_value=fake_sim):
            result = injector.top_k_triple_extractor(question, triples, k=3)
        assert len(result) == 3

    def test_clamps_k_when_fewer_triples_available(self, injector):
        question = np.random.rand(1, 16)
        triples = np.random.rand(2, 16)
        fake_sim = np.random.rand(1, 2)
        with patch("kaping.entity_injection.cosine_similarity", return_value=fake_sim):
            result = injector.top_k_triple_extractor(question, triples, k=10)
        assert len(result) == 2

    def test_returns_subset_of_triples(self, injector):
        question = np.random.rand(1, 16)
        triples = np.random.rand(5, 16)
        fake_sim = np.random.rand(1, 5)
        with patch("kaping.entity_injection.cosine_similarity", return_value=fake_sim):
            result = injector.top_k_triple_extractor(question, triples, k=3)
        for row in result:
            assert any(np.array_equal(row, triples[i]) for i in range(len(triples)))

    def test_random_mode_returns_k_items(self, injector):
        question = np.random.rand(1, 16)
        triples = ["t1", "t2", "t3", "t4", "t5"]
        result = injector.top_k_triple_extractor(question, triples, k=3, random=True)
        assert len(result) == 3
    ...
    def test_most_similar_triple_is_selected(self, injector):
        ...
        # similarity scores: first triple has highest score
        fake_sim = np.array([[0.9, 0.3, 0.1]])
        with patch("kaping.entity_injection.cosine_similarity", return_value=fake_sim):
            result = injector.top_k_triple_extractor(question, triples, k=1)
        assert np.array_equal(result[0], triples[0])
